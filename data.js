...
const AppState = {
  ...
  applyAdjustment(adjId) {
    const adj = this.adjustments.find(a => a.id === adjId);
    if (!adj || adj.status === 'done') return false;
    const prod = this.getProductById(adj.productId);
    if (prod) {
      const oldQty = prod.onHand;
      prod.onHand = adj.counted;
      adj.diff = adj.counted - oldQty;
      adj.status = 'done';
      this.ledger.unshift({
        id: this.ledger.length + 1,
        date: new Date().toISOString().slice(0,16).replace('T',' '),
        type: 'adjustment', ref: adj.id, productId: prod.id,
        qty: adj.diff,
        fromLoc: this.getLocationById(adj.locationId)?.name || 'Stock',
        toLoc: adj.diff < 0 ? 'Scrap' : 'Stock',
        note: adj.reason
      });
    }
    return true;
